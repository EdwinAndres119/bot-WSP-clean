...
2. **`protocolTimeout` de Puppeteer** (`src/wa/client.js`): está en `180000` (3 min), a propósito. Se probó en `600000` (10 min) y los cuelgues reales (ej. un `downloadMedia()` trabado) tardaban hasta 10 min en salir como error, bloqueando todo ese tiempo. 3 min cubre cómodo la ventana de polling de sync (~30s) y queda por debajo del `CHAT_TIMEOUT_MS` (5 min) propio de la app. No lo subas de nuevo sin una razón concreta.
