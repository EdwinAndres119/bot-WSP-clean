...
- Los chats vacíos/fallidos **sí quedan persistidos** en `extraction_runs` (`empty_chats` / `failed_chats`, jsonb) y se consultan por `/api/runs` o directo en Supabase — pero **no salen en el CSV** de `/api/export`, que solo exporta la tabla `mensajes`. Si alguien de negocio pregunta "¿por qué este número no trajo nada?", esa respuesta está en el panel, no en el Excel.
